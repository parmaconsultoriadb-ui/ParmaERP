...
def insert_row(table: str, data: Dict[str, Any]) -> Dict[str, Any]:
    sb = get_supabase()
    res = sb.table(table).insert(data).select("*").single().execute()
    return res.data

def update_row(table: str, row_id: Any, data: Dict[str, Any], id_col: str = "id") -> Dict[str, Any]:
    sb = get_supabase()
    res = sb.table(table).update(data).eq(id_col, row_id).select("*").single().execute()
    return res.data
